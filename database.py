...
def check_login(username, password):
    if not supabase:
        return None
    try:
        response = supabase.table("Users").select("fullname, role").eq("Username", username).eq("Password", password).execute()
        data = response.data
...
def get_latest_tdm(msyt):
    ...
    try:
        ...
        t_res = supabase.table("tdm_history").select("*").eq("msyt", msyt).order("tdm_date", desc=True).limit(1).execute()
...
def delete_tdm_block(msyt, tdm_date):
    ...
    try:
        ...
        st.error(f"Lỗi khi xóa ca TDM: {e}")
...
def import_patients_from_dataframe(df):
    ...
    try:
        ...
        st.error(f"Lỗi import bệnh nhân: {e}")
...
def save_patient_info(msyt, gender, weight, height, age, is_cf):
    """Lưu hoặc cập nhật thông tin hành chính bệnh nhân lên Cloud"""
    if not supabase:
        return
    try:
        data = {
        ...
        st.error(f"Lỗi lưu thông tin bệnh nhân lên Cloud: {e}")
